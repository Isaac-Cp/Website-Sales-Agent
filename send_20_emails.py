...
send_20_emails.py
-----------------
Sends up to 20 real sales emails to leads already stored in leads.db.
Uses the existing Mailer + llm_helper + config infrastructure.
Run with: python send_20_emails.py
"""
import sys
import sqlite3
import time
...
for lead in candidates:
    ...
    if is_followup:
        ...
        for attempt in range(3):
            try:
                # Use generate_email_content which has fallback to spintax if LLM fails
                subject, body = llm_helper.generate_email_content(
                    name, niche, city, strategy=strategy, 
                    audit_issues=audit_issues, description=description, 
                    reviews=reviews, first_name=None, country="US"
                )
                provider = "llm_fallback" if not config.GROQ_API_KEY else "llm"
                persona = "spintax" if not config.GROQ_API_KEY else "generated"
                hook_type = "audit"
                if not body.strip():
                ...
                # Set outreach_result to indicate success
                outreach_result = {"subject": subject, "email": body}
                break
            except Exception as e:
                if attempt < 2:
...
print(f"\n[SUMMARY] Sent {sent_count} out of {TARGET_COUNT} emails.")
print(f"Unique businesses emailed today (including this run): {unique_daily_count + sent_count}")
